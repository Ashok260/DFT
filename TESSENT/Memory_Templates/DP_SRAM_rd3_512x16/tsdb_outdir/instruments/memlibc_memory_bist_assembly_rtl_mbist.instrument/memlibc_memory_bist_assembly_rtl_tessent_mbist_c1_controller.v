...
-          Version: 2022.4                                                       -
-       Created on: Tue Jun 10 16:36:28 IST 2025                                 -
----------------------------------------------------------------------------------
...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller (
  ...
  output wire [0:0] BIST_TEST_PORT,
...
wire [0:0] PORT_COUNTER_INT;
wire       PORT_COUNTER_FREEZE;
wire       LAST_PORT;
wire       NEXT_PORT;
wire       PORT_COUNTER_SI;
wire       PORT_COUNTER_SO;
...
//------------------------
    ...
    assign INIT_DATA_GEN_REGS       = ~(SELECT_COMMON_DATA_PAT_REG | (PORT_COUNTER_FREEZE));
  ...
  assign BIST_HOLD_R              = BIST_HOLD_INT;            
    ...
    assign NEXT_ALGO                = ALGO_DONE & (~LAST_PORT);
...
assign CMP_EN = CMP_EN_INT;
    ...
    assign PORT_COUNTER_SI          = CTL_COMP_SO;
    ...
    always @ (posedge BIST_CLK_INT or negedge MBISTPG_ASYNC_RESETN) begin
       if (~MBISTPG_ASYNC_RESETN)
          ...
          BIST_SO_PIPELINE <= PORT_COUNTER_SO;
    end
...
assign ENABLE_ERR_CNT_CAP = PORT_COUNTER_FREEZE | (PORT_COUNTER_INT ==0);
    ...
    wire PORT_COUNTER_CLEAR_DEFAULT;
    assign PORT_COUNTER_CLEAR_DEFAULT = CLEAR_DEFAULT;
    memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_port_counter MBISTPG_PORT_COUNTER (
       .BIST_CLK                    ( BIST_CLK_INT               ), // i
       .BIST_EN                     ( MBISTPG_EN_INT             ), // i
       .SI                          ( PORT_COUNTER_SI            ), // i
       .BIST_SHIFT_SHORT            ( BIST_SHIFT_SHORT           ), // i
       .BIST_HOLD                   ( BIST_HOLD_R_INT            ), // i
       .BIST_ASYNC_RESETN           ( MBISTPG_ASYNC_RESETN       ), // i
       .ALGO_MODE                   ( MBISTPG_ALGO_MODE_INT      ), // i
       .ESOE_RESET                  ( ESOE_RESET                 ), // i
       .BIST_CLEAR_DEFAULT          ( PORT_COUNTER_CLEAR_DEFAULT                ), // i
       .NEXT_PORT                   ( NEXT_PORT                  ), // o
       .SO                          ( PORT_COUNTER_SO            ), // o
       .PORT_COUNTER                ( PORT_COUNTER_INT           ), // o
       .PORT_COUNTER_FREEZE         ( PORT_COUNTER_FREEZE        ), // o
       .LAST_PORT                   ( LAST_PORT                  )  // o
    );
 
    assign BIST_TEST_PORT           = PORT_COUNTER_INT & {1{BIST_RUN_INT}};
    assign NEXT_PORT                = NEXT_ALGO;
...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_port_counter (
  input wire BIST_CLK,
  input wire BIST_EN,
  input wire SI,
  input wire BIST_SHIFT_SHORT,
  ...
  input wire NEXT_PORT,
  input wire ESOE_RESET,
  output wire SO,
  output reg [0:0] PORT_COUNTER,
  output reg PORT_COUNTER_FREEZE,
  output wire LAST_PORT
);
 
    wire   [0:0]     PORT_COUNTER_MAX;
    wire             PORT_COUNTER_RST;
    wire             PORT_COUNTER_FREEZE_RST;

    assign PORT_COUNTER_MAX = 1'b1;

    assign LAST_PORT = PORT_COUNTER_FREEZE | (PORT_COUNTER == PORT_COUNTER_MAX) | (ALGO_MODE != 2'b00);

    assign PORT_COUNTER_FREEZE_RST = (~BIST_HOLD) & (~BIST_SHIFT_SHORT) & BIST_CLEAR_DEFAULT;

    // synopsys sync_set_reset "PORT_COUNTER_FREEZE_RST"
    // synopsys async_set_reset "BIST_ASYNC_RESETN"
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        if (~BIST_ASYNC_RESETN)
            PORT_COUNTER_FREEZE <= 1'b0;
        else
        if (PORT_COUNTER_FREEZE_RST) begin
            PORT_COUNTER_FREEZE <= 1'b0;
        end else begin
            if (BIST_SHIFT_SHORT) begin
                PORT_COUNTER_FREEZE <= SI;
            end
        end
    end

    assign PORT_COUNTER_RST = ((~BIST_HOLD) & (~BIST_SHIFT_SHORT) & BIST_CLEAR_DEFAULT) | ESOE_RESET;
 
    // synopsys sync_set_reset "PORT_COUNTER_RST"
    // synopsys async_set_reset "BIST_ASYNC_RESETN"
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        if (~BIST_ASYNC_RESETN)
            PORT_COUNTER <= 1'b0;
        else
        if (PORT_COUNTER_RST) begin
            PORT_COUNTER <= 1'b0;
        end else begin
            if (BIST_SHIFT_SHORT) begin
                PORT_COUNTER <= PORT_COUNTER_FREEZE;
            end else begin
                if ((~BIST_HOLD) & NEXT_PORT & (~PORT_COUNTER_FREEZE)) begin
                    if (LAST_PORT) begin
                        PORT_COUNTER <= 1'b0;
                    end else begin
                        PORT_COUNTER <= PORT_COUNTER_INC(PORT_COUNTER);
                    end
                end
            end
        end
    ...
    assign SO = PORT_COUNTER[0];

    function automatic [0:0] PORT_COUNTER_INC;
        input [0:0] PORT_COUNTER;
    begin
        PORT_COUNTER_INC[0] = ~PORT_COUNTER[0];
    end
    endfunction

endmodule // memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_port_counter
