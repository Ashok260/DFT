...
module memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_assembly(LV_TM, 
                                                                             ...
                                                                             tck, 
                                                                             so);
  input  [23:0] m1_inst_I1, m1_inst_I2;
  input  [8:0] m1_inst_A1, m1_inst_A2;
  input  LV_TM, MEM_BYPASS_EN, SCAN_SHIFT_EN, MCP_BOUNDING_EN, clk_BIST_CLK, 
         m1_inst_WEB1, m1_inst_WEB2, m1_inst_OEB1, m1_inst_OEB2, m1_inst_CSB1, 
         m1_inst_CSB2, reset, ijtag_select, si, capture_en, shift_en, 
         update_en, tck;
  output [23:0] m1_inst_O1, m1_inst_O2;
  output BIST_ON, BIST_DONE, BIST_GO, so;

  wire [23:0] m1_interface_instance_I1, m1_interface_instance_I2;
  wire [8:0] m1_interface_instance_A1, m1_interface_instance_A2;
  wire [6:0] BIST_ROW_ADD;
  wire [2:1] BIST_SETUP_ts1;
  wire [1:0] BIST_COL_ADD, BIST_WRITE_DATA, BIST_EXPECT_DATA;
  wire [0:0] toBist, bistEn, BIST_TEST_PORT;
  wire BIST_HOLD, BIST_SETUP, BIST_SELECT_TEST_DATA, to_controllers_tck, 
       ...
       MBISTPG_RESET_REG_SETUP2, WEB1, WEB2, CSB1, CSB2;

  DPRAM_512x24 m1_inst(
      .CE1(clk_BIST_CLK), .CE2(clk_BIST_CLK), .WEB1(WEB1), .WEB2(WEB2), .OEB1(m1_inst_OEB1), 
      .OEB2(m1_inst_OEB2), .CSB1(CSB1), .CSB2(CSB2), .A1(m1_interface_instance_A1), 
      .A2(m1_interface_instance_A2), .I1(m1_interface_instance_I1), .I2(m1_interface_instance_I2), 
      .O1(m1_inst_O1), .O2(m1_inst_O2)
  );
  ...
  memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_controller_inst(
      ...
      .BIST_RUN_TO_COLLAR0(BIST_RUN_TO_COLLAR0), .BIST_TEST_PORT(BIST_TEST_PORT), 
      .MBISTPG_GO(BIST_GO), .MBISTPG_DONE(BIST_DONE), .BIST_ON_TO_COLLAR(BIST_ON)
  );
  memlibc_memory_bist_assembly_rtl_tessent_mbist_c1_interface_m1 m1_interface_instance(
      .WEB1_IN(m1_inst_WEB1), .WEB2_IN(m1_inst_WEB2), .CSB1_IN(m1_inst_CSB1), .CSB2_IN(m1_inst_CSB2), 
      .A1_IN(m1_inst_A1[8:0]), .A2_IN(m1_inst_A2[8:0]), .I1_IN(m1_inst_I1[23:0]), 
      .I2_IN(m1_inst_I2[23:0]), .O1(m1_inst_O1), .O2(m1_inst_O2), .TCK(to_interfaces_tck), 
      .BIST_CMP(BIST_CMP), .INCLUDE_MEM_RESULTS_REG(INCLUDE_MEM_RESULTS_REG), .BIST_WRITEENABLE(BIST_WRITEENABLE), 
      .BIST_OUTPUTENABLE(BIST_OUTPUTENABLE), .BIST_SELECT(BIST_SELECT), .BIST_COL_ADD(BIST_COL_ADD), 
      .BIST_ROW_ADD(BIST_ROW_ADD), .BIST_TEST_PORT(BIST_TEST_PORT), .BIST_WRITE_DATA(BIST_WRITE_DATA), 
      ...
      .BIST_RUN(BIST_RUN_TO_COLLAR0), .BIST_ASYNC_RESETN(BIST_ASYNC_RESET), .BIST_CLK(clk_BIST_CLK), 
      ...
      .ERROR_CNT_ZERO(ERROR_CNT_ZERO), .WEB1(WEB1), .WEB2(WEB2), .CSB1(CSB1), .CSB2(CSB2), 
      .A1(m1_interface_instance_A1), .A2(m1_interface_instance_A2), .I1(m1_interface_instance_I1), 
      .I2(m1_interface_instance_I2), .SCAN_OBS_FLOPS(), .BIST_SO(BIST_SO), .BIST_GO(BIST_GO_ts1)
  );
endmodule
