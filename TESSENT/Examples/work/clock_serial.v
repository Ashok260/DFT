...
// Filename       : clock_serial.v
// Idstamp        : 2022.4:827d:4603:4:91ca
// Fault          : STUCK
// Coverage       : 7.51(TC) 7.26(FC)
// Date           : Tue Apr 22 18:28:44 2025
//
// Begin_Verify_Section 
//   format            = Verilog 
//   top_module_name   = CLOCK_clock_serial_v_ctl 
//   serial_flag       = ON 
...
module CLOCK_clock_serial_v_ctl;
...
`ifdef VCD
    $sformat(_vcd_dump_file_name, "clock_serial.v.dump");
...
`ifdef UTVCD
    $sformat(_utvcd_dump_file_name, "clock_serial.v.dump");
    if(_change_out_path) begin 
...
`ifdef debussy
    $sformat(_fsdb_dump_file_name, "clock_serial.v.fsdb");
    if(_change_out_path) begin 
...
`ifdef QWAVE
    $sformat(_qwave_dump_file_name, "clock_serial.v.qwave.db");
    if(_change_out_path) begin 
...
initial begin 
   if(_change_path) begin 
     $sformat(_new_filename,"%0s/clock_serial.v.po.name",_new_path); 
     $display("Loading %0s\n", _new_filename ); 
     $readmemh(_new_filename,_nam_obus,25,0); 
   end 
   else begin
     $display("Loading clock_serial.v.po.name");
     $readmemh("clock_serial.v.po.name",_nam_obus,25,0);
   end 
end 
      ...
      if ((_msk_obus[_bit_count] === 1'b1) &&
        ...
        if (_write_DIAG_file == 1) begin
          if (_DIAG_file_header == 0) begin
            if ((_start_pat > -1) && (_end_pat > -1)) begin
              $sformat(_tmp_filename, "clock_serial.v_%0d_%0d.fail",
                       _start_pat, _end_pat);
            end
            else if (_start_pat > -1) begin
              $sformat(_tmp_filename, "clock_serial.v_%0d.fail",
                       _start_pat);
            end
            else if (_end_pat > -1) begin
              $sformat(_tmp_filename, "clock_serial.v__%0d.fail",
                       _end_pat);
            end
            else begin
              $sformat(_tmp_filename, "clock_serial.v.fail");
            end
            if(_change_out_path) begin 
            ...
            if(_change_out_path) begin 
              $fwrite(_diag_file, "// This File is simulation generated (%0s/clock_serial.v)\n", _new_out_path);
            end
            else begin
              $fwrite(_diag_file, "// This File is simulation generated (clock_serial.v)\n");
            end
        ...
        if (_write_MASK_file == 1) begin
          if (_MASK_file_header == 0) begin
            if ((_start_pat > -1) && (_end_pat > -1)) begin
              $sformat(_tmp_filename, "clock_serial.v_%0d_%0d.mask",
                       _start_pat, _end_pat);
            end
            else if (_start_pat > -1) begin
              $sformat(_tmp_filename, "clock_serial.v_%0d.mask",
                       _start_pat);
            end
            else if (_end_pat > -1) begin
              $sformat(_tmp_filename, "clock_serial.v__%0d.mask",
                       _end_pat);
            end
            else begin
              $sformat(_tmp_filename, "clock_serial.v.mask");
            end
            if(_change_out_path) begin 
              $sformat(_tmp_filename, "%0s/%0s", _new_out_path, _tmp_filename);
            end
...
//  $display("Reading checkpoint clock_serial.v.dat");
//  $restart("clock_serial.v.dat");
end
...
if ($value$plusargs("CONFIG=%0s", _cfg_file_name)) begin
  ...
  _cfg_file_name = "clock_serial.v.cfg";
end
...
if(_change_path) begin 
  $sformat(_new_filename,"%0s/clock_serial.v.cfg",_new_path); 
  _config_file = $fopen(_new_filename, "r");
end
else begin
  _config_file = $fopen("clock_serial.v.cfg", "r");
end
...
//  $display("Writing checkpoint clock_serial.v.dat");
//  $save("clock_serial.v.dat");
